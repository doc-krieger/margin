...
function textNode(text: string, marks: MarkdownMark[] = []): MarkdownNode {
  return marks.length > 0 ? { type: "text", text, marks } : { type: "text", text };
}

function sameMarks(left: MarkdownMark[] | undefined, right: MarkdownMark[]): boolean {
  return JSON.stringify(left ?? []) === JSON.stringify(right);
}

function pushText(nodes: MarkdownNode[], text: string, marks: MarkdownMark[]): void {
...
function parseInline(text: string, activeMarks: MarkdownMark[] = []): MarkdownNode[] {
  ...
  for (let index = 0; index < text.length;) {
    ...
    else if (remainder.startsWith("**")) marker = "**";
    else if (remainder.startsWith("*")) marker = "*";
    else if (remainder.startsWith("__")) marker = "__";
    else if (remainder.startsWith("_")) marker = "_";
    ...
    if (marker) {
      ...
      const marks = marker === "***" || marker === "___"
        ? [...activeMarks, { type: "strong" as const }, { type: "em" as const }]
        : marker.length === 2
          ? [...activeMarks, { type: "strong" as const }]
          : [...activeMarks, { type: "em" as const }];
...
function splitTableRow(line: string): string[] {
  ...
  if (!trimmed.includes("|")) throw new MarkdownParseError("A table row must contain a pipe separator.");
  const cells: string[] = [];
  let current = "";
  let escaped = false;
...
function isTableStart(lines: string[], index: number): boolean {
  ...
  if (!row.includes("|") || index + 1 >= lines.length) return false;
  try {
...
function parseTable(lines: string[], start: number): { node: MarkdownNode; next: number } {
  const header = splitTableRow(lines[start] ?? "");
  if (!isTableSeparator(lines[start + 1] ?? "", header.length)) {
    throw new MarkdownParseError("Malformed table separator.", start + 2);
  }
  const rows: MarkdownNode[] = [{ type: "tableRow", content: header.map((cell) => ({ type: "tableHeader", content: [{ type: "paragraph", content: parseInline(cell) }] })) }];
  let index = start + 2;
  while (index < lines.length && !isBlank(lines[index] ?? "") && (lines[index] ?? "").includes("|")) {
    const cells = splitTableRow(lines[index] ?? "");
    if (cells.length !== header.length) throw new MarkdownParseError("All table rows must have the same number of cells.", index + 1);
...
function parseList(lines: string[], start: number, ordered: boolean): { node: MarkdownNode; next: number } {
  const itemPattern = ordered ? orderedPattern : bulletPattern;
  const first = (lines[start] ?? "").match(itemPattern);
  if (!first) throw new MarkdownParseError("Malformed list item.", start + 1);
  ...
  while (index < lines.length) {
    ...
    if (expectedOrder !== undefined) expectedOrder += 1;
    index += 1;
  }
  return {
    node: {
...
function parseBlocks(lines: string[]): MarkdownNode[] {
  ...
  while (index < lines.length) {
    ...
    if (heading) {
      ...
      if (value.length === 0) throw new MarkdownParseError("Headings must contain text.", index + 1);
    ...
    if (paragraphLines.length === 0) throw new MarkdownParseError("Could not classify Markdown block.", index + 1);
...
function parseErrorFeature(error: unknown): UnsupportedFeature {
...
export function parseMarkdown(source: string): MarkdownParseResult {
  ...
  try {
    ...
    const document: MarkdownDocument = { type: "doc", content: parseBlocks(lines) };
